...
def _build_episode_prompt(series, ep_num, unique, villain, trending_hint=""):
    ...
    if is_last:
        ...
        ending_rule = (
            "【本集非最終集｜選擇必須緊扣正文，最重要】\n"
            "結尾要收喺一個『主角即場要做決定』嘅具體處境，而呢個處境必須喺正文最後一兩段"
            "清楚寫咗出嚟（邊個場景、面對邊個人、發生咩事）。\n"
            "跟住畀嘅兩個選擇，一定要係主角對住『你啱啱寫嗰一幕』嘅兩個唔同做法——\n"
            "❌ 嚴禁引入正文從未出現過嘅新元素（唔好無端端整張照片、一封信、一個新角色或新地點出嚟做選擇）。\n"
            "✅ 讀者一睇就明：呢兩個選擇就係主角面對眼前呢件事、而家可以行嘅兩條路。\n"
            "全文最尾另起三行輸出控制碼，格式一字不差、每行三個尖括號：\n"
            "<<<NEXT: 一句下集懸念種子，20字內，緊扣呢一幕>>>\n"
            "<<<CA: 承接上面處境嘅選擇一，主角一個具體行動，12字內>>>\n"
            "<<<CB: 承接上面處境嘅選擇二，同A明顯不同，12字內>>>\n"
            "示範（假設正文最後一幕係：對方撂低一句挑釁就轉身走）：\n"
            "<<<NEXT: 佢背後嘅人即將浮面>>>\n<<<CA: 當場叫住佢對質>>>\n<<<CB: 忍住、暗中查佢底細>>>")
