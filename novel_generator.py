...
GENRES = [
    ...
    {"name": "重生年代稱霸", "channel": "M", "weight": 2, "era": "nian_dai", "desc": "主角生於1970-80年代，在物資匱乏、被人踩低的起點，憑眼光和手腕一步步佈局；第二幕跨越至現代，白手起家的他早已稱霸一方，讓當年所有嘲笑過他的人親眼見證"},
...
MALE_FIRST = [
    ...
    # 年代文風（1970-80s 常見名）
    "建國", "衛東", "志遠", "國強", "明德", "正陽", "勝利", "建軍",
    "永強", "光明", "保國", "紅軍", "忠誠", "衛民", "開陽", "慶豐",
    # 現代都市新潮
...
MALE_ERA_NAMES = ["建國", "衛東", "志遠", "國強", "明德", "正陽", "勝利", "建軍",
                  "永強", "光明", "保國", "紅軍", "忠誠", "衛民", "開陽", "慶豐"]
FEMALE_ERA_NAMES = ["秀蘭", "玉珍", "翠芬", "麗萍", "桂花", "春梅", "秀英", "鳳蘭"]
# 古言宮廷男名
...
def generate_character(channel="M", genre_name=""):
    ...
    # 姓氏：20% 複姓（年代文和古言例外，一律用單姓）
    if not is_era and not is_court and random.random() < 0.20:
    ...
    # 名字池選擇
    if is_era:
        firstname = random.choice(MALE_ERA_NAMES if gender == "男" else FEMALE_ERA_NAMES)
    elif is_court:
...
WRITING_STYLES = [
    ...
    "年代質感風：用具體時代細節（食物、物價、歌曲、街道）建立真實感，慢熱但越看越捨不得放手，像老照片慢慢洗出來的顏色",
